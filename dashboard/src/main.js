...
import { acknowledgeAlert, archiveAlert, approveAgent, bulkAcknowledgeAlerts, bulkArchiveAlerts, changeUserPassword, createGroup, createMaintenanceWindow, createUser, deleteAgent, deleteMaintenanceWindow, deleteUser, disableUser, enableUser, fetchAlerts, fetchMetricHistory, fetchSession, fetchSettings, fetchUptimeReport, loadDashboardData, login, logout, rejectAgent, requestAgentStatus, restartAgentCollectors, resumeAgent, setAgentCollectorConfig, setAgentDescription, setAgentGroups, setMaintenance, updateSettings, } from './api';
...
function SettingsPage({ runAction, }) {
    const [draft, setDraft] = React.useState({ webhook_url: '', offline_after_seconds: 120, escalation_timeout_seconds: 300 });
    ...
    return (_jsxs("main", { className: "table-wrap management-wrap", children: [_jsx("div", { className: "management-header", children: _jsx("h1", { children: "Settings" }) }), !loaded ? _jsx("div", { className: "banner", children: "Loading settings..." }) : null, loaded ? (_jsxs("form", { className: "settings-form", onSubmit: (event) => {
                ...
                }, children: [_jsxs("div", { className: "form-grid", children: [_jsxs("label", { children: ["Webhook URL", _jsx("input", { placeholder: "https://hooks.example.com/\u2026", type: "url", value: draft.webhook_url, onChange: (e) => setDraft((d) => ({ ...d, webhook_url: e.target.value })) })] }), _jsxs("label", { children: ["Offline after (seconds)", _jsx("input", { min: 10, step: 1, type: "number", value: draft.offline_after_seconds, onChange: (e) => setDraft((d) => ({ ...d, offline_after_seconds: Number(e.target.value) })) })] }), _jsxs("label", { children: ["Escalation timeout (seconds)", _jsx("input", { min: 60, step: 1, type: "number", value: draft.escalation_timeout_seconds, onChange: (e) => setDraft((d) => ({ ...d, escalation_timeout_seconds: Number(e.target.value) })) })] })] }), _jsx("div", { className: "button-row", children: _jsx("button", { className: "text-button", type: "submit", children: "Save Settings" }) })] })) : null] }));
}
