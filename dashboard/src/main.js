...
import { acknowledgeAlert, archiveAlert, approveAgent, bulkAcknowledgeAlerts, bulkArchiveAlerts, changeUserPassword, createGroup, createMaintenanceWindow, createSilence, createUser, createRunbook, createView, deleteAgent, deleteMaintenanceWindow, deleteRunbook, deleteSilence, deleteUser, deleteView, fetchRunbooks, disableUser, enableUser, fetchAgentHistory, fetchAlerts, fetchLogMatches, fetchMetricHistory, fetchSession, fetchSettings, fetchUptimeReport, fetchViews, loadDashboardData, login, logout, rejectAgent, requestAgentStatus, restartAgentCollectors, resumeAgent, setAgentCollectorConfig, setAgentDescription, sendReport, setAgentGroups, setMaintenance, updateSettings, updateView, } from './api';
...
function ViewsPage({ agents, busyAction, operator, runAction, session, views, onOpenView, }) {
    ...
    const canEdit = (v) => operator && (v.owner_username === session.username || session.role === 'admin');
    return (_jsxs("main", { className: "table-wrap", children: [_jsx("div", { className: "table-container", children: _jsxs("table", { children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Name" }), _jsx("th", { children: "Owner" }), _jsx("th", { children: "Agents" }), _jsx("th", { children: "Visibility" }), _jsx("th", { children: "Created" }), _jsx("th", {})] }) }), _jsxs("tbody", { children: [views.map((v) => (_jsx("tr", { children: editId === v.view_id ? (_jsxs(_Fragment, { children: [_jsx("td", { colSpan: 4, children: _jsxs("div", { className: "form-grid", style: { display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }, children: [_jsx("input", { value: editName, placeholder: "View name", onChange: (e) => setEditName(e.target.value) }), _jsxs("label", { style: { display: 'flex', gap: '0.25rem', alignItems: 'center' }, children: [_jsx("input", { type: "checkbox", checked: editPublic, onChange: (e) => setEditPublic(e.target.checked) }), "Public"] }), _jsx("select", { multiple: true, size: Math.min(agents.length, 6), style: { minWidth: '14rem' }, value: editAgentIds, onChange: (e) => setEditAgentIds(Array.from(e.target.selectedOptions, (o) => o.value)), children: agents.map((a) => _jsx("option", { value: a.id, children: a.name }, a.id)) })] }) }), _jsx("td", { children: new Date(v.created_at).toLocaleDateString() }), _jsxs("td", { children: [_jsx(ActionButton, { busy: busyAction === `view:save:${v.view_id}`, label: "Save", icon: _jsx(Check, { size: 14 }), onClick: () => void runAction(`view:save:${v.view_id}`, () => updateView(v.view_id, editName, editAgentIds, editPublic).then(() => { })).then(() => setEditId(null)) }), _jsx(ActionButton, { busy: false, label: "Cancel", icon: _jsx(X, { size: 14 }), onClick: () => setEditId(null) })] })] })) : (_jsxs(_Fragment, { children: [_jsx("td", { children: _jsx("strong", { children: v.name }) }), _jsx("td", { children: v.owner_username }), _jsxs("td", { children: [v.agent_ids.length, " agent", v.agent_ids.length !== 1 ? 's' : ''] }), _jsx("td", { children: v.is_public ? 'Public' : 'Private' }), _jsx("td", { children: new Date(v.created_at).toLocaleDateString() }), _jsxs("td", { children: [_jsx(ActionButton, { busy: false, label: "Open", icon: _jsx(Search, { size: 14 }), onClick: () => onOpenView(v.view_id) }), canEdit(v) ? _jsx(ActionButton, { busy: false, label: "Edit", icon: _jsx(SlidersHorizontal, { size: 14 }), onClick: () => startEdit(v) }) : null, canEdit(v) ? (_jsx(ActionButton, { busy: busyAction === `view:delete:${v.view_id}`, danger: true, label: "Delete", icon: _jsx(Trash2, { size: 14 }), onClick: () => void runAction(`view:delete:${v.view_id}`, () => deleteView(v.view_id)) })) : null] })] })) }, v.view_id))), views.length === 0 ? (_jsx("tr", { children: _jsx("td", { colSpan: 6, style: { textAlign: 'center', padding: '1rem' }, children: "No views yet. Create one below." }) })) : null] })] }) }), operator ? (_jsx("div", { className: "table-container", style: { marginTop: '1rem' }, children: _jsxs("form", { onSubmit: (e) => {
                        e.preventDefault();
