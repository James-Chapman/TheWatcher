...
function Dashboard() {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("header", { className: "topbar", children: [_jsxs("div", { className: "logo", children: [_jsx("span", { className: "logo-dot" }), "THEWATCHER"] }), _jsx("nav", { className: "nav-tabs", "aria-label": "Dashboard views", children: NAV_ITEMS.map((item) => {
                            if (item.requiresOperator && !operator)
                                return null;
                            if (item.requiresGlobalOperator && !(global && operator))
                                return null;
                            const label = item.view === 'views' ? `Views${customViews.length > 0 ? ` (${customViews.length})` : ''}` : item.label;
                            return _jsx(Tab, { view: view, target: item.view, setView: setView, label: label }, item.view);
                        }) }), _jsxs("div", { className: "topbar-meta", children: [_jsx("span", { children: _jsx("strong", { children: loadedAt ? loadedAt.toUTCString().replace('GMT', 'UTC') : 'Not synced' }) }), _jsxs("span", { children: [session.username, " ", _jsx("strong", { children: accountTypeLabel(session.role) })] }), _jsx("button", { className: "icon-button", onClick: () => void refresh(), "aria-label": "Refresh dashboard", children: _jsx(RefreshCw, { size: 14 }) }), _jsx("button", { className: "icon-button", onClick: () => {
                            ...
                            : agents, error: error, expanded: expanded, groupFilter: overviewGroupFilter, groups: groups, global: global, loading: loading, setExpanded: setExpanded, setGroupFilter: setOverviewGroupFilter })] })) : null, view === 'views' ? (_jsx(ViewsPage, { agents: agents, busyAction: busyAction, operator: operator, runAction: runAction, session: session, views: customViews, onOpenView: (viewId) => { setActiveViewId(viewId); setView('monitoring'); }, groups: groups })) : null, view === 'agents' ? (_jsx(AgentManagement, { agents: agents, busyAction: busyAction, error: error, groups: groups, loading: loading, operator: operator, onConfigure: setConfigAgentId, pending: global && operator ? pending : [], showPending: global && operator, runAction: runAction })) : null, configAgent ? (_jsx(AgentConfigModal, { agent: configAgent, busy: busyAction === `${configAgent.id}:collector_config`, onClose: () => setConfigAgentId(null), operator: operator && canManageAgent(configAgent), global: global, groups: groups, runAction: runAction })) : null, view === 'alerts' ? (_jsx(AlertsPage, { agents: agents, alerts: toDisplayAlerts(allAlerts, agents), busyAction: busyAction, operator: operator, runAction: runAction })) : null, view === 'maintenance' ? (_jsx(MaintenanceWindowsPage, { agents: agents, busyAction: busyAction, global: global, groups: groups, operator: operator, runAction: runAction, windows: maintenanceWindows })) : null, view === 'silences' ? (_jsx(SilencesPage, { agents: agents, busyAction: busyAction, global: global, operator: operator, runAction: runAction, silences: silences })) : null, view === 'users' ? _jsx(UsersPage, { busyAction: busyAction, groups: groups, runAction: runAction, session: session, users: users }) : null, view === 'settings' ? _jsx(SettingsPage, { runAction: runAction }) : null] }));
}
...
function MonitoringTable({ agents, error, expanded, global, groupFilter, groups, loading, setExpanded, setGroupFilter, }) {
...
function AgentManagement({ agents, busyAction, error, groups, loading, onConfigure, operator, pending, showPending, runAction, }) {
    const [descDrafts, setDescDrafts] = React.useState({});
    return (_jsxs("main", { className: "table-wrap management-wrap", children: [error ? _jsxs("div", { className: "banner error", children: ["API error: ", error] }) : null, loading ? _jsx("div", { className: "banner", children: "Loading dashboard data..." }) : null, showPending ? _jsx(PendingEnrollments, { agents: pending, busyAction: busyAction, groups: groups, runAction: runAction }) : null, _jsx("div", { className: "management-header", children: _jsx("h1", { children: "Agent Management" }) }), _jsx("div", { className: "table-container", children: _jsxs("table", { className: "management-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Agent" }), _jsx("th", { children: "Platform" }), _jsx("th", { children: "State" }), _jsx("th", { children: "Last Seen" }), _jsx("th", { children: "Groups" }), _jsx("th", { children: "Commands" }), _jsx("th", { children: "Configure" }), _jsx("th", { children: "Delete" })] }) }), _jsx("tbody", { children: agents.map((agent) => {
                                ...
                                return (_jsxs("tr", { children: [_jsx("td", { children: _jsxs("div", { className: "agent-config-cell", children: [_jsx(AgentIdentity, { id: agent.id, name: agent.name, secondary: `${agent.ipAddress || 'IP unknown'} / ${agent.id}` }), _jsxs("div", { className: "settings-grid desc-settings", children: [_jsx("input", { disabled: !operator, placeholder: "Description\u2026", value: descValue, onChange: (event) => setDescDrafts((current) => ({ ...current, [agent.id]: event.target.value })) }), _jsx(ActionButton, { busy: busyAction === `${agent.id}:desc`, disabled: !operator || descValue === agent.description, icon: _jsx(Check, { size: 14 }), label: "Set", onClick: () => runAction(`${agent.id}:desc`, () => setAgentDescription(agent.id, descValue)) })] })] }) }), _jsx("td", { children: agent.platform }), _jsx("td", { children: _jsx(StatusDot, { color: agent.status, label: colorLabel(agent.status) }) }), _jsx("td", { className: "uptime", children: agent.lastSeen > 0 ? `${Math.floor((Date.now() - agent.lastSeen) / 60000)}m` : 'never' }), _jsx("td", { children: agent.groupIds.map((id) => groups.find((group) => group.group_id === id)?.name ?? id).join(', ') || 'No group' }), _jsx("td", { children: _jsxs("div", { className: "button-row", children: [_jsx(ActionButton, { busy: busyAction === `${agent.id}:${maintenanceCommand}`, disabled: !manageable, icon: agent.maintenance ? _jsx(Play, { size: 14 }) : _jsx(Pause, { size: 14 }), label: maintenanceActionLabel(agent), onClick: () => runAction(`${agent.id}:${maintenanceCommand}`, () => agent.maintenance ? resumeAgent(agent.id) : setMaintenance(agent.id, 'operator request', 0)) }), _jsx(ActionButton, { busy: busyAction === `${agent.id}:restart`, disabled: !manageable, icon: _jsx(RotateCw, { size: 14 }), label: "Restart", onClick: () => runAction(`${agent.id}:restart`, () => restartAgentCollectors(agent.id)) }), _jsx(ActionButton, { busy: busyAction === `${agent.id}:status`, disabled: !manageable, icon: _jsx(Search, { size: 14 }), label: "Status", onClick: () => runAction(`${agent.id}:status`, () => requestAgentStatus(agent.id)) })] }) }), _jsx("td", { children: _jsx(ActionButton, { busy: busyAction === `${agent.id}:configure`, disabled: !manageable, icon: _jsx(SlidersHorizontal, { size: 14 }), label: "Configure", onClick: () => onConfigure(agent.id) }) }), _jsx("td", { children: _jsx(ActionButton, { busy: busyAction === `${agent.id}:delete`, danger: true, disabled: !manageable, icon: _jsx(Trash2, { size: 14 }), label: "Delete", onClick: () => runAction(`${agent.id}:delete`, () => deleteAgent(agent.id)) }) })] }, agent.id));
                            }) })] }) })] }));
}
...
function AgentConfigModal({ agent: agentProp, busy, global, groups, onClose, operator, runAction, }) {
                ...
                void runAction(`${agent.id}:collector_config`, () => setAgentCollectorConfig(agent.id, draft)).then((saved) => {
                    if (saved)
                        onClose();
                                        ...
                                        _jsxs("div", { className: "process-row", children: [_jsxs("label", { className: "toggle-line", title: "Enables or disables this process watch without deleting it.", children: [_jsx("input", { type: "checkbox", checked: process.enabled, onChange: (event) => updateProcess(index, (current) => ({ ...current, enabled: event.target.checked })) }), _jsx("span", { children: "Enabled" })] }), _jsx("input", { title: "Exact executable or process name expected to be running on this agent.", value: process.name, placeholder: "exact executable name", onChange: (event) => updateProcess(index, (current) => ({ ...current, name: event.target.value })) }), _jsx(NumberField, { help: "Expected number of matching processes. Alerts are raised when fewer are running.", label: "Expected", min: 1, value: process.expected_count, onChange: (value) => updateProcess(index, (current) => ({ ...current, expected_count: value })) }), _jsx(ActionButton, { busy: false, danger: true, icon: _jsx(Trash2, { size: 14 }), label: "Remove", onClick: () => updateConfig((config) => ({ ...config, processes: config.processes.filter((_, currentIndex) => currentIndex !== index) })) })] }, `process:${index}`))), draft.collector_config.processes.length === 0 ? _jsx("div", { className: "empty-config", children: "No process watches configured." }) : null] })] }), _jsxs("section", { className: "wide-section", children: [_jsxs("div", { className: "section-title-row", children: [_jsx("h2", { children: "Log Watches" }), _jsx(ActionButton, { busy: false, icon: _jsx(Plus, { size: 14 }), label: "Log Watch", onClick: () => updateConfig((config) => ({
                                                ...config,
                                                logs: [...(config.logs ?? []), { path: '', pattern: '', indicator_name: '', severity: 'red', enabled: true }],
                                            })) })] }), _jsxs("div", { className: "config-list", children: [(draft.collector_config.logs ?? []).map((log, index) => (_jsxs("div", { className: "process-row", children: [_jsxs("label", { className: "toggle-line", title: "Enables or disables this log watch without deleting it.", children: [_jsx("input", { type: "checkbox", checked: log.enabled, onChange: (event) => updateLog(index, (current) => ({ ...current, enabled: event.target.checked })) }), _jsx("span", { children: "Enabled" })] }), _jsx("input", { title: "Absolute path to the log file the agent should scan.", value: log.path, placeholder: "/var/log/syslog", onChange: (event) => updateLog(index, (current) => ({ ...current, path: event.target.value })) }), _jsx("input", { title: "Regular expression pattern that identifies matching log lines.", value: log.pattern, placeholder: "regex pattern", onChange: (event) => updateLog(index, (current) => ({ ...current, pattern: event.target.value })) }), _jsx("input", { title: "Dashboard indicator name to use when this log pattern matches.", value: log.indicator_name, placeholder: "indicator name", onChange: (event) => updateLog(index, (current) => ({ ...current, indicator_name: event.target.value })) }), _jsxs("select", { title: "Alert severity raised when this log pattern matches.", value: log.severity, onChange: (event) => updateLog(index, (current) => ({ ...current, severity: event.target.value })), children: [_jsx("option", { value: "yellow", children: "Warning" }), _jsx("option", { value: "amber", children: "Degraded" }), _jsx("option", { value: "red", children: "Critical" })] }), _jsx(ActionButton, { busy: false, danger: true, icon: _jsx(Trash2, { size: 14 }), label: "Remove", onClick: () => updateConfig((config) => ({ ...config, logs: (config.logs ?? []).filter((_, currentIndex) => currentIndex !== index) })) })] }, `log:${index}`))), (draft.collector_config.logs ?? []).length === 0 ? _jsx("div", { className: "empty-config", children: "No log watches configured." }) : null] })] })] }), _jsxs("div", { className: "modal-actions", children: [_jsx("button", { className: "text-button", type: "button", onClick: onClose, children: "Cancel" }), _jsx("button", { className: "text-button", type: "submit", disabled: !operator || busy, children: busy ? '...' : 'Save' })] })] }) }));
}
...
function AnomalyInputs({ anomaly, onChange, }) {
    return (_jsxs("div", { className: "anomaly-inputs", children: [_jsx(NumberField, { help: "Multiplier above the learned baseline required before anomaly status is raised. Zero disables anomaly detection.", label: "Anomaly multiplier (0=off)", min: 0, step: 0.1, value: anomaly.multiplier, onChange: (value) => onChange('multiplier', value) }), _jsx(NumberField, { help: "Number of hours of recent samples used to calculate the normal baseline for anomaly detection.", label: "Baseline hours", min: 1, value: anomaly.baseline_window_hours, onChange: (value) => onChange('baseline_window_hours', value) })] }));
}
function ThresholdInputs({ onChange, thresholds, }) {
    return (_jsxs("div", { className: "threshold-inputs", children: [_jsx(NumberField, { help: "Usage percent that raises warning status for this item.", label: "Warn", min: 1, value: thresholds.warning_percent, onChange: (value) => onChange('warning_percent', value) }), _jsx(NumberField, { help: "Usage percent that raises degraded status for this item.", label: "Degrade", min: 1, value: thresholds.degraded_percent, onChange: (value) => onChange('degraded_percent', value) }), _jsx(NumberField, { help: "Usage percent that raises critical status for this item.", label: "Critical", min: 1, value: thresholds.critical_percent, onChange: (value) => onChange('critical_percent', value) })] }));
}
...
function NetworkThresholdInputs({ onChange, thresholds, }) {
    return (_jsxs("div", { className: "threshold-inputs", children: [_jsx(NumberField, { help: "Network throughput in megabits per second that raises warning status.", label: "Warn Mb/s", min: 1, value: thresholds.warning_mbps, onChange: (value) => onChange('warning_mbps', value) }), _jsx(NumberField, { help: "Network throughput in megabits per second that raises degraded status.", label: "Degrade Mb/s", min: 1, value: thresholds.degraded_mbps, onChange: (value) => onChange('degraded_mbps', value) }), _jsx(NumberField, { help: "Network throughput in megabits per second that raises critical status.", label: "Critical Mb/s", min: 1, value: thresholds.critical_mbps, onChange: (value) => onChange('critical_mbps', value) })] }));
}
function PendingEnrollments({ agents, busyAction, groups, runAction }) {
    const defaultGroup = groups[0]?.group_id;
    return (_jsxs("section", { className: "embedded-management", children: [_jsx("div", { className: "management-header", children: _jsx("h1", { children: "Pending Enrollments" }) }), _jsx("div", { className: "table-container", children: _jsxs("table", { className: "management-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Agent" }), _jsx("th", { children: "Platform" }), _jsx("th", { children: "Last Seen" }), _jsx("th", { children: "Actions" })] }) }), _jsxs("tbody", { children: [agents.map((agent) => (_jsxs("tr", { children: [_jsx("td", { children: _jsx(AgentIdentity, { id: agent.id, name: agent.name, secondary: `${agent.ipAddress || 'IP unknown'} / ${agent.id}` }) }), _jsx("td", { children: agent.platform }), _jsx("td", { className: "uptime", children: agent.lastSeen > 0 ? `${Math.floor((Date.now() - agent.lastSeen) / 60000)}m` : 'never' }), _jsx("td", { children: _jsxs("div", { className: "button-row", children: [_jsx(ActionButton, { busy: busyAction === `${agent.id}:approve`, icon: _jsx(Check, { size: 14 }), label: "Approve", onClick: () => runAction(`${agent.id}:approve`, () => approveAgent(agent.id, defaultGroup ? [defaultGroup] : [])) }), _jsx(ActionButton, { busy: busyAction === `${agent.id}:reject`, icon: _jsx(X, { size: 14 }), label: "Reject", onClick: () => runAction(`${agent.id}:reject`, () => rejectAgent(agent.id)) })] }) })] }, agent.id))), agents.length === 0 ? _jsx("tr", { children: _jsx("td", { colSpan: 4, className: "uptime", children: "No pending enrollments." }) }) : null] })] }) })] }));
}
...
function renderInlineMarkdown(text) {
    ...
    while ((match = linkPattern.exec(text)) !== null) {
        ...
        nodes.push(href ? (_jsx("a", { href: href, rel: "noopener noreferrer", target: "_blank", children: match[1] }, `${match.index}:${href}`)) : (match[1]));
...
function RunbookMarkdown({ markdown }) {
    const blocks = parseMarkdownBlocks(markdown);
    if (blocks.length === 0)
        return _jsx("div", { className: "banner", children: "No runbook has been configured for this agent." });
    return (_jsx("div", { className: "runbook-markdown", children: blocks.map((block, index) => {
            if (block.type === 'heading') {
                const Heading = `h${block.level}`;
                return _jsx(Heading, { children: renderInlineMarkdown(block.text) }, index);
            }
            if (block.type === 'list') {
                return (_jsx("ul", { children: block.items.map((item, itemIndex) => _jsx("li", { children: renderInlineMarkdown(item) }, itemIndex)) }, index));
            }
            if (block.type === 'code')
                return _jsx("pre", { children: _jsx("code", { children: block.text }) }, index);
            return _jsx("p", { children: renderInlineMarkdown(block.text) }, index);
        }) }));
}
function RunbookModal({ agentName, markdown, onClose }) {
    return (_jsx("div", { className: "modal-backdrop", onClick: onClose, children: _jsxs("div", { className: "modal-panel runbook-modal", onClick: (event) => event.stopPropagation(), children: [_jsxs("div", { className: "modal-header", children: [_jsxs("h2", { children: ["Runbook - ", agentName] }), _jsx("button", { className: "icon-button", onClick: onClose, "aria-label": "Close runbook", children: _jsx(X, { size: 14 }) })] }), _jsx("div", { className: "runbook-body", children: _jsx(RunbookMarkdown, { markdown: markdown }) })] }) }));
}
function AlertsPage({ agents, alerts, busyAction, operator, runAction, }) {
    ...
    const [runbookTarget, setRunbookTarget] = React.useState(null);
...
function MaintenanceWindowsPage({ agents, busyAction, global, groups, operator, runAction, windows, }) {
    ...
    React.useEffect(() => {
        if (!global && agentId === '*' && agents.length > 0)
...
function SilencesPage({ agents, busyAction, global, operator, runAction, silences, }) {
    const [agentId, setAgentId] = React.useState('*');
...
function UsersPage({ busyAction, groups, runAction, session, users, }) {
    ...
    return (_jsxs("main", { className: "table-wrap management-wrap", children: [pwUserId !== null ? (_jsx("div", { className: "modal-backdrop", onClick: () => setPwUserId(null), children: _jsxs("div", { className: "modal-panel", onClick: (e) => e.stopPropagation(), children: [_jsxs("div", { className: "modal-header", children: [_jsx("h2", { children: "Change Password" }), _jsx("button", { className: "icon-button", onClick: () => setPwUserId(null), children: _jsx(X, { size: 14 }) })] }), _jsxs("label", { children: ["New password", _jsx("input", { type: "password", autoFocus: true, value: pwValue, onChange: (e) => setPwValue(e.target.value) })] }), _jsxs("div", { className: "button-row modal-actions", children: [_jsx("button", { className: "text-button secondary", onClick: () => setPwUserId(null), children: "Cancel" }), _jsx("button", { className: "text-button", disabled: !pwValue, onClick: () => {
                        ...
                        }, children: [_jsx("input", { "aria-label": "Username", placeholder: "Username", value: username, onChange: (event) => setUsername(event.target.value) }), _jsx("input", { "aria-label": "Password", placeholder: "Password", type: "password", value: password, onChange: (event) => setPassword(event.target.value) }), _jsxs("select", { "aria-label": "Account type", value: role, onChange: (event) => setRole(event.target.value), children: [globalAccount ? _jsx("option", { value: "global_admin", children: accountTypeLabel('global_admin') }) : null, globalAccount ? _jsx("option", { value: "global_operator", children: accountTypeLabel('global_operator') }) : null, globalAccount ? _jsx("option", { value: "global_viewer", children: accountTypeLabel('global_viewer') }) : null, _jsx("option", { value: "group_admin", children: accountTypeLabel('group_admin') }), _jsx("option", { value: "group_operator", children: accountTypeLabel('group_operator') }), _jsx("option", { value: "group_viewer", children: accountTypeLabel('group_viewer') })] }), _jsxs("select", { "aria-label": "Group", value: groupId, onChange: (event) => setGroupId(Number(event.target.value)), children: [globalAccount ? _jsx("option", { value: 0, children: "No group" }) : null, groups.map((group) => (_jsx("option", { value: group.group_id, children: group.name }, group.group_id)))] }), _jsx(ActionButton, { busy: busyAction === 'user:create', icon: _jsx(Plus, { size: 14 }), label: "User", onClick: () => undefined, type: "submit" })] }) : null] }), _jsx("div", { className: "table-container", children: _jsxs("table", { className: "management-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "User" }), _jsx("th", { children: "Type" }), _jsx("th", { children: "Groups" }), _jsx("th", { children: "State" }), _jsx("th", { children: "Actions" })] }) }), _jsx("tbody", { children: users.map((user) => {
                                const canChangePw = admin || currentUser?.user_id === user.user_id;
                                return (_jsxs("tr", { className: user.disabled ? 'row-archived' : '', children: [_jsx("td", { children: user.username }), _jsx("td", { children: accountTypeLabel(user.role) }), _jsx("td", { children: user.group_ids.map((id) => groups.find((group) => group.group_id === id)?.name ?? id).join(', ') }), _jsx("td", { children: user.disabled ? 'Disabled' : 'Active' }), _jsx("td", { children: _jsxs("div", { className: "button-row", children: [canChangePw ? (_jsx(ActionButton, { busy: busyAction === `user:${user.user_id}:pw`, icon: _jsx(SlidersHorizontal, { size: 14 }), label: "Password", onClick: () => { setPwUserId(user.user_id); setPwValue(''); } })) : null, globalAdmin && !user.built_in ? (user.disabled ? (_jsx(ActionButton, { busy: busyAction === `user:${user.user_id}:enable`, icon: _jsx(Play, { size: 14 }), label: "Enable", onClick: () => runAction(`user:${user.user_id}:enable`, () => enableUser(user.user_id)) })) : (_jsx(ActionButton, { busy: busyAction === `user:${user.user_id}:disable`, icon: _jsx(Pause, { size: 14 }), label: "Disable", onClick: () => runAction(`user:${user.user_id}:disable`, () => disableUser(user.user_id)) }))) : null, globalAdmin && !user.built_in ? (_jsx(ActionButton, { busy: busyAction === `user:${user.user_id}:delete`, danger: true, icon: _jsx(Trash2, { size: 14 }), label: "Delete", onClick: () => runAction(`user:${user.user_id}:delete`, () => deleteUser(user.user_id)) })) : null] }) })] }, user.user_id));
                            }) })] }) })] }));
}
